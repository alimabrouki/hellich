...
function Hellich () {
  ...
  const aboutIntroText = 'جسدك أمانة… قوِّه اليوم قبل أن يضعف غدا.'
  const aboutIntroWords = useMemo(
    () => aboutIntroText.split(/\s+/),
    [aboutIntroText]
  )
  const wordDelayMs = 45

  const handleMenuOpen = useCallback((state: boolean) => {
    setMenuOpen(state)
  }, [])
